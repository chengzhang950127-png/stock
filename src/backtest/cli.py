...
Usage::
    ...
    # Calibration mode — uses _calibration_strategies.BuyAndHoldStrategy
    ...
    # Production strategies (V0.2+) — selected from src/strategies/__init__.py
    # registry; V0.1 has no production strategies registered yet.
...
```
{
  "account_initial_cash": "100000.00",
  "account_final_nav": "...",
  "metrics": { total_return, annual_return, sharpe, sortino,
               max_drawdown, calmar, win_rate, avg_holding_days },
  "trades": [...],
  "trade_count": <int>,
  "total_fees": "...",
  "snapshots_count": <int>,
        ...
        for row in reader:
            ...
            bars.append(
                PriceBar(
                    ...
                    close=Decimal(_pick(row, "close", "Close")),
                    adj_close=Decimal(_pick(row, "adj_close", "Adj Close", "close", "Close")),
                    volume=int(float(_pick(row, "volume", "Volume"))),
                )
            )
...
def _synthetic_bars(ticker: str, start: date, end: date) -> list[PriceBar]:
    """Deterministic ramp — diagnostic-only fallback when --csv is omitted."""
    bars: list[PriceBar] = []
    cur = start
...
def _serialize_result(
    ...
    """Render BacktestResult as a JSON-friendly dict for review tools."""

    def _money(d: Decimal) -> str:
        return f"{d:.2f}"

    total_fees = sum((t.fee for t in result.trades), start=Decimal("0"))
    final_nav = (
        result.performance_snapshots[-1].nav
        if result.performance_snapshots
        else result.account_final.cash
    )

    return {
        "account_initial_cash": _money(initial_cash),
        "account_final_cash": _money(result.account_final.cash),
        ...
        "unexecuted_signal_count": len(result.unexecuted_signals),
    }
...
def cmd_run(args: argparse.Namespace) -> int:
    if args.strategy != "buy_and_hold":
        sys.stderr.write(
            f"Strategy {args.strategy!r} is not registered. V0.1 only ships "
            "buy_and_hold (calibration). Production strategies arrive in V0.2.\n"
        )
        return 2
    if not args.calibration_mode:
        sys.stderr.write(
            "buy_and_hold is a calibration strategy. Pass --calibration-mode to "
            "acknowledge it should not be used as a production strategy.\n"
        )
        return 2
    ...
    strategy = BuyAndHoldStrategy(ticker=ticker)
    ...
    result = engine.run()
    payload = _serialize_result(result, INITIAL_CAPITAL)
...
def main(argv: list[str] | None = None) -> int:
    ...
    run.add_argument(
        "--calibration-mode",
        action="store_true",
        help="Acknowledge buy_and_hold is a calibration strategy, not production",
    )
