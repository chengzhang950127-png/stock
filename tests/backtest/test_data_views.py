"""PointInTimeDataView — confirms strict ``<= as_of`` filtering."""
...
def test_get_bars_filters_strictly_to_as_of() -> None:
    ...
    assert returned[-1].date == date(2024, 1, 5)


def test_get_bars_excludes_strictly_future_dates() -> None:
    bars = _bars_jan_2024()
    view = PointInTimeDataView({"TEST": bars}, as_of=date(2024, 1, 3))

    returned = view.get_bars("TEST")

    future_dates = [b.date for b in returned if b.date > date(2024, 1, 3)]
    assert future_dates == []
...
def test_get_bar_on_returns_none_for_missing_date_within_window() -> None:
    """A date <= as_of but with no bar (non-trading day) returns None."""
    bars = _bars_jan_2024()
    view = PointInTimeDataView({"TEST": bars}, as_of=date(2024, 1, 10))

    assert view.get_bar_on("TEST", date(2023, 12, 25)) is None
    # also: unknown symbol returns None
...
def test_get_universe_returns_provided_list() -> None:
    ...
    view = PointInTimeDataView({}, as_of=date(2024, 1, 1), universe=universe)
...
def test_lookahead_error_message_contains_context() -> None:
    """Lookahead errors must say WHICH symbol and WHICH date — debugging cheaply."""
    bars = _bars_jan_2024()
    view = PointInTimeDataView({"TEST": bars}, as_of=date(2024, 1, 3))
...
def test_simulated_cheating_strategy_is_blocked() -> None:
    ...
    def cheating_strategy_lookup() -> PriceBar | None:
        # Strategy mistakenly asks for tomorrow's bar — exactly what #B1 forbids.
